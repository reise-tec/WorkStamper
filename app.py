...
def get_freee_employee_id_by_email(email):
    ...
    headers = { "Authorization": f"Bearer {FREEEE_API_TOKEN}", "FREEEE-COMPANY-ID": str(FREEEE_COMPANY_ID) }
    params = {"email": email}
    try:
...
def call_freee_time_clock(employee_id, clock_type):
    ...
    headers = { "Authorization": f"Bearer {FREEEE_API_TOKEN}", "FREEEE-COMPANY-ID": str(FREEEE_COMPANY_ID), "Content-Type": "application/json" }
    data = { "company_id": int(FREEEE_COMPANY_ID), "type": clock_type, "base_date": datetime.date.today().isoformat(), "datetime": datetime.datetime.now().isoformat() }
    try:
        response = requests.post(url, headers=headers, json=data)
        response.raise_for_status()
        return True
    except Exception as e:
        logging.error(f"freee打刻APIエラー: {e}")
        return False
...
def get_freee_leave_types():
    ...
    try:
        ...
        leave_types = [ {"id": t["id"], "name": t["name"]} for t in templates if t.get("category") == "leave" ]
        return leave_types
    except Exception as e:
...
def add_event_to_google_calendar(summary, start_date, end_date):
    ...
    try:
        ...
        event = { 'summary': summary, 'start': {'date': start_date, 'timeZone': 'Asia/Tokyo'}, 'end': {'date': end_date_for_api, 'timeZone': 'Asia/Tokyo'} }
        service.events().insert(calendarId=GOOGLE_CALENDAR_ID, body=event).execute()
        return True
    except HttpError as error:
        logging.error(f"Google Calendar APIエラー: {error}")
        return False
    except Exception as e:
...
def handle_clock_out_command(ack, body, client):
    ...
    if not employee_id:
        client.chat_postMessage(channel=user_id, text="エラー: freee従業員情報が見つかりません")
        return
    if call_freee_time_clock(employee_id, "clock_out"):
        client.chat_postMessage(channel=user_id, text="退勤打刻完了。お疲れ様でした！")
    else:
        client.chat_postMessage(channel=user_id, text="エラー: freee打刻失敗")
...
def handle_leave_request_command(ack, body, client):
    ...
    try:
        client.views_open(trigger_id=body["trigger_id"], view={"type": "modal", "callback_id": "leave_request_view", "title": {"type": "plain_text", "text": "休暇申請"},"submit": {"type": "plain_text", "text": "申請"}, "blocks": [{"type": "input", "block_id": "leave_type_block", "label": {"type": "plain_text", "text": "休暇種別"},"element": { "type": "static_select", "action_id": "leave_type_select", "placeholder": {"type": "plain_text", "text": "休暇種別を選択"}, "options": options }}, {"type": "input", "block_id": "start_date_block", "label": {"type": "plain_text", "text": "開始日"},"element": {"type": "datepicker", "action_id": "start_date_picker", "initial_date": datetime.datetime.now().strftime('%Y-%m-%d')}}, {"type": "input", "block_id": "end_date_block", "label": {"type": "plain_text", "text": "終了日"},"element": {"type": "datepicker", "action_id": "end_date_picker", "initial_date": datetime.datetime.now().strftime('%Y-%m-%d')}}, {"type": "input", "block_id": "reason_block", "label": {"type": "plain_text", "text": "詳細理由（任意）"}, "optional": True,"element": {"type": "plain_text_input", "action_id": "reason_input", "placeholder": {"type": "plain_text", "text": "例：通院のため"}, "multiline": True}}]})
    except Exception as e:
...
def handle_clock_in_submission(ack, body, client, view):
    ...
    if call_freee_time_clock(employee_id, "clock_in"):
        client.chat_postMessage(channel=user_id, text=f"出勤打刻完了。（勤務形態: {location}）\n今日も一日頑張りましょう！")
    else:
        client.chat_postMessage(channel=user_id, text="エラー: freee打刻失敗")
...
def handle_leave_request_submission(ack, body, client):
    ...
    if add_event_to_google_calendar(summary, start_date, end_date):
        ...
        client.chat_postMessage(channel=user_id, text="カレンダー登録失敗。アクセストークンが古い可能性あり。")
